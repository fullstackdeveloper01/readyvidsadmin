...
class FrontEndController extends Controller
{
    ...
    public function getDownload(){
        ...
        $videoarray=array_reverse(explode('/',$_GET['url']));
     
        $file= public_path(). "/uploads/make_video/".$videoarray[1]."/".$videoarray[0];
    ...
    public function getDownload(){
        ...
        $destinationVideoPath=public_path()."/uploads/download_video/".$videoarray[1];
        if( !is_dir( $destinationVideoPath ) ) mkdir( $destinationVideoPath, 0755, true );
        $video_url = "/uploads/download_video/".$videoarray[1]."/".time().'.'.$format;
        $output_path= public_path().$video_url;
                            
        $output= shell_exec("ffmpeg -i $input -vf scale=$quality:-1 $output_path 2>&1");
        
        $name=$_GET['name'].".".$format;
        $headers = array(
        ...
        return response()->download($output_path,$name);
    }
