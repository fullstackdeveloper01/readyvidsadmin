...
@section('content')
    ...
    @include('template.partials.header', ['title' => __('Edit template')])
...
.bg-orange{background-color:#feb06a;}
    ...
    <div class="container-fluid mt--7">
                ...
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        ...
                        <div class="pl-lg-4">
                            ...
                            <form method="post" action="{{ route('template.store') }}" autocomplete="off" enctype="multipart/form-data">
                                ...
                                <div class="pl-lg-4">
                                    ...
                                    <div class="row">
                                        <div class="col-md-3 form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                            ...
                                            <input type="text" name="template_name" id="template_name" class="form-control form-control-alternative{{ $errors->has('template_name') ? ' is-invalid' : '' }}" required value={{$template->template_name}}>
                                        ...
                                        <div class="col-md-2 form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                            ...
                                            @foreach($typeList as $res)
                                                <option value="{{$res->id}}" @if($template->name==$res->id) selected @endif>{{$res->type}}</option>
                                            @endforeach
                                        ...
                                        <div class="col-md-2 form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                            ...
                                            @foreach($ratioList as $res)
                                                <option value="{{$res->id}}" @if($template->ratio==$res->id) selected @endif>{{$res->name}}</option>
                                            @endforeach
                                        ...
                                        <div class="col-md-2 form-group{{ $errors->has('type') ? ' has-danger' : '' }}">
                                            ...
                                            <select name="type" id="type" class="form-control form-control-alternative{{ $errors->has('type') ? ' is-invalid' : '' }}" value="{{old('type')}}" required>
                                                <option value=""> -- </option>
                                                <option value="with_image" @if($template->type=="with_image") selected @endif>With Image</option>
                                                <option value="without_image" @if($template->type=="without_image") selected @endif>Without Image</option>
                                        ...
                                        <div class="col-md-4 form-group{{ $errors->has('pattern') ? ' has-danger' : '' }}">
                                            ...
                                            @foreach($patternList as $res)
                                                <option value="{{$res->id}}" @if($template->pattern==$res->id) selected @endif>{{$res->name}}</option>
                                            @endforeach
        ...
        <script type="text/javascript">
            ...
            $('#name').change(function(){
                ...
                //$('#preview_name').html($("#name option:selected").text());
                  $.ajax({
                    ...
                    url: base_url+'/pattern/gettemplatepattern/'+template_name+'/'+image_type+'/'+ratio,
            ...
            function getFont(id,val){
             
              var class_name =id.id;
                var style=$('.line'+class_name).attr('style');
                 var font =id.value;
                 var fontarray=font.split(':');
                 var fontstyle= fontarray[1].split(' ');
                 if(fontstyle.length>1){
                    if(style.indexOf("font-family: 'Hind'")== 0){
                        style +=';font-style:'+fontstyle[1];
                    }
                 }
                 if(style.indexOf("font-family: 'Hind'")== 0){
                 ...
                 $('.line'+class_name).attr('style',style);
            }
            function getFontBackgroundColor(id,val){
                
                var style=$('.linefonts'+id+' div').attr('style');
                style+=';background-color:'+$('#textbg'+id).val();
                
                //$('.linefonts'+id).attr('style',style);
                $('.linefonts'+id+' div').attr('style',style);
            }
            function getFontColor(id,val){
               
                var style=$('.linefonts'+id+' div').attr('style');
                style+=';color:'+$('#color'+id).val();
                
                //$('.linefonts'+id+' span').attr('style',style);
                $('.linefonts'+id+' div').attr('style',style);
            }
            $("#saveImage").click(function(){
             ...
             var template_type= $('#name option:selected').html();
             var lineno= template_type.split(' ');
             if(lineno[0]>0){
                 for(var counter=1;counter<=lineno[0];counter++){
                     $(".linefonts"+counter+' div').html('{text'+counter+'}');
                 }
                 $('.imgborder').attr('src','{img}');
             }
             var bodystyle= $("#backgroundc").attr('style');
