@extends('layouts.app', ['name' => __('Video Type Management')])

@section('content')
    @include('section.partials.header', ['name' => __('Add Video Type')])
            ...
            <div class="col-xl-12 order-xl-1">
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        ...
                        <div class="pl-lg-4">
                            <form method="post" action="{{ route('section.store') }}" autocomplete="off" enctype="multipart/form-data">
                                @csrf
                                </div>
                                <div class="pl-lg-4">
                                    <div class="col-md-6 form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                        <label class="form-control-label" for="name">{{ __('Type') }}</label>
                                        <input type="text" name="name" id="name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Type') }}" value="" required>
                                        @if ($errors->has('name'))
