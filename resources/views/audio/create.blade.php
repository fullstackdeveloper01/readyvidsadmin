...
@section('content')
    @include('subCategories.partials.header', ['name' => __('Add Image')])
                            ...
                            <form method="post" action="{{ route('image.store') }}" autocomplete="off" enctype="multipart/form-data">
                                ...
                                <div class="pl-lg-4">
                                    <!-- <div class="form-group{{ $errors->has('image_name') ? ' has-danger' : '' }}">
                                        <label class="form-control-label" for="image_name">{{ __('Image Name') }}</label>
                                        <input name="image_name" id="image_name" class="form-control form-control-alternative{{ $errors->has('image_name') ? ' is-invalid' : '' }}" required>
