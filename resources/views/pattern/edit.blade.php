...
@section('content')
    @include('pattern.partials.header', ['title' => __('Edit Pattern')])
        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        <div class="pl-lg-4">
                            <form method="post" action="{{ route('pattern.update', $pattern) }}" autocomplete="off" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                <hr />
                                ...
                                <div class="pl-lg-4">
                                    <div class="row">
                                        <div class="col-md-6 form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                            ...
                                            <input type="text" name="name" id="name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Pattern name') }}" value="{{$pattern->name}}" required>
                                            @if ($errors->has('name'))
                                        ...
                                        <div class="col-md-6 form-group{{ $errors->has('template_type') ? ' has-danger' : '' }}">
                                            ...
                                            @foreach($typeList as $res)
                                                <option value="{{$res->id}}" @if($pattern->template_type==$res->id) selected @endif>{{$res->type}}</option>
                                            @endforeach
                                        ...
                                        <div class="col-md-6 form-group{{ $errors->has('ratio') ? ' has-danger' : '' }}">
                                            ...
                                            @foreach($ratioList as $res)
                                                <option value="{{$res->id}}" @if($pattern->ratio==$res->id) selected @endif>{{$res->name}}</option>
                                            @endforeach
                                        ...
                                        <div class="col-md-6 form-group{{ $errors->has('type') ? ' has-danger' : '' }}">
                                            ...
                                            <select name="image_type" id="image_type" class="form-control form-control-alternative{{ $errors->has('image_type') ? ' is-invalid' : '' }}" value="{{old('type')}}" required>
                                                <option value=""> -- </option>
                                                <option value="with_image" @if($pattern->image_type=='with_image') selected @endif>With Image</option>
                                                <option value="without_image" @if($pattern->image_type=='without_image') selected @endif>Without Image</option>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <img width="100px" height="100px" src="{{ asset("{$pattern->image}") }}">
                                        </div>
